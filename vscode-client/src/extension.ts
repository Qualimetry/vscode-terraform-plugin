...
const SONAR_TOKEN_SECRET_KEY = 'terraform.sonar.token';
...
function applyRulesFilter(
    ...
    for (const d of diagnostics) {
        ...
        if (rulesReplaceDefaults) {
            ...
            // d.code carries the clickable help target ({ value, target }); copying it whole preserves codeDescription.
            diag.code = d.code;
            if (entry.severity) {
 ...
 * terraformAnalyzer.rules + terraformAnalyzer.rulesReplaceDefaults. Throws on failure.
 */
async function syncRulesFromSonarQube(
    ...
    if (profiles.length === 0) {
        throw new Error('No Terraform quality profiles found on this SonarQube server.');
    }
    const profileKey = resolveProfileKey(profiles, config.profileNameOrKey);
    if (!profileKey) {
        throw new Error(
            `No matching Terraform profile for "${config.profileNameOrKey}". ` +
...
async function autoSyncRulesOnStartup(context: vscode.ExtensionContext): Promise<void> {
    const autoSync = vscode.workspace
        .getConfiguration('terraformAnalyzer')
    ...
    try {
        ...
        vscode.window.showWarningMessage(
            `Qualimetry Terraform: SonarQube rule sync failed: ${message}`
        );
    }
}
...
async function importRulesFromSonarQube(context: vscode.ExtensionContext): Promise<void> {
    ...
    const profileNameOrKey = await vscode.window.showInputBox({
        ...
        prompt: 'Profile name or key (e.g. "Qualimetry Terraform" or the profile key)',
        value: lastProfile,
