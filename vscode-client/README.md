...
Static analysis for Terraform (HCL) `.tf` and `.tf.json` files. This extension shares the same rule set as the [Qualimetry Terraform Analyzer for SonarQube](https://github.com/Qualimetry/sonarqube-terraform-plugin). It reports misconfigurations, security findings, style and convention violations, and correctness errors. Rule keys and severities match the SonarQube plugin for consistency between editor and pipeline.
